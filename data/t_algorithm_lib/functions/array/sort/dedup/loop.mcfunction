...
execute store result score #tal.array.dedup.list_count tal.str_parser run data get storage t_algorithm_lib:array get_element_index.return
##如果不重复，则添加
execute if score #tal.array.dedup.list_count tal.str_parser matches 0 run data modify storage t_algorithm_lib:array dedup.return append from storage t_algorithm_lib:array dedup.list2[0]
...
execute if score #tal.array.dedup.list_count tal.str_parser matches 1.. run function t_algorithm_lib:array/sort/dedup/loop