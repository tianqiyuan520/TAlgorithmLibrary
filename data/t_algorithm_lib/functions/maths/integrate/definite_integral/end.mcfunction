...
#
execute store result storage t_algorithm_lib:array double_to_array.num double 1.0 run scoreboard players get #tal.maths.integrate.samples tal.input
function t_algorithm_lib:array/double_to_array/start
...
execute if score #tal.maths.integrate.remind tal.input matches 1 run tellraw @a ["运算结束\nresult: ",{"nbt":"integrate.sum","storage": "t_algorithm_lib:maths","color": "green"}]
execute if score #tal.maths.integrate.remind tal.input matches 1 run playsound entity.player.levelup voice @a ~ ~ ~ 1 2
...
# execute store result storage t_algorithm_lib:maths integrate.sum double 0.000001 run scoreboard players operation #tal.maths.integrate.sum tal.input /= #tal.maths.integrate.samples tal.input